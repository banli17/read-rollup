...
export async function rollupInternal(
	...
	// 设置 timer, 如果有 perf, 需要记录各个阶段的时间、内存等
...
async function getInputOptions(
	...
	const rawPlugins = getSortedValidatedPlugins(
		'options',
		await normalizePluginOption(rawInputOptions.plugins) // 标准化插件 -> 会拍平所有的插件(promise)
	);
	// 合并处理 option, 用户传入, 默认的, 插件里的
	...
	// 标准化插件的名字, 如果没有插件名 name 字段, 会设置插件名: at position n
